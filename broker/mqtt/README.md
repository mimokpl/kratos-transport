...
b := mqtt.NewBroker(
    ...
    mqtt.WithClientId("kratos-client"),
...
// 发布
b.Publish(ctx, "sensor/temperature", broker.NewMessage(msg),
    mqtt.WithPublishQos(1),
)

// 订阅
sub, _ := b.Subscribe("sensor/temperature", handler, binder,
...
```go
b.Publish(ctx, "sensor/config", broker.NewMessage(msg),
    mqtt.WithPublishQos(1),
    mqtt.WithPublishRetained(true),
)
```
...
| 选项 | 说明 |
|------|------|
| `mqtt.WithCleanSession(enable)` | 清除会话（默认 true） |
| `mqtt.WithAuth(username, password)` | 用户名密码认证 |
| `mqtt.WithClientId(id)` | 客户端 ID |
| `mqtt.WithAutoReconnect(enable)` | 自动重连 |
| `mqtt.WithKeepAlive(d)` | 心跳间隔 |
| `mqtt.WithMaxReconnectInterval(d)` | 最大重连间隔 |
| `mqtt.WithConnectTimeout(d)` | 连接超时 |
| `mqtt.WithWriteTimeout(d)` | 写入超时 |
| `mqtt.WithPingTimeout(d)` | Ping 超时 |
| `mqtt.WithProtocolVersion(v)` | 协议版本（3/4/5） |
| `mqtt.WithResumeSubs(enable)` | 恢复订阅 |
| `mqtt.WithOrderMatters(enable)` | 消息有序性 |
| `mqtt.WithOnConnect(cb)` | 连接成功回调 |
| `mqtt.WithOnDisconnect(cb)` | 断开连接回调 |

### Publish 选项

| 选项 | 说明 |
|------|------|
| `mqtt.WithPublishQos(qos)` | QoS 级别（0/1/2） |
| `mqtt.WithPublishRetained(retained)` | Retained 消息 |

### Subscribe 选项

| 选项 | 说明 |
|------|------|
| `mqtt.WithSubscribeQos(qos)` | QoS 级别（0/1/2） |
