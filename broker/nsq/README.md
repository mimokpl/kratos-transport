...
sub, _ := b.Subscribe("test-topic", handler, binder,
    broker.WithSubscribeQueueName("my-channel"),
    nsq.WithConcurrentHandlers(4),
)
```
...
b := nsq.NewBroker(
    ...
    nsq.WithLookupdAddress([]string{"127.0.0.1:4160"}),
)

// 延迟消息
b.Publish(ctx, "test-topic", broker.NewMessage(msg),
    nsq.WithDeferredPublish(30*time.Minute),
)
```
...
| 选项 | 说明 |
|------|------|
| `nsq.WithLookupdAddress(addrs)` | nsqlookupd 地址列表 |
| `nsq.WithConsumerOptions(opts)` | 原生 Consumer 配置选项 |

### Publish 选项

| 选项 | 说明 |
|------|------|
| `nsq.WithAsyncPublish(ch)` | 异步发布，ch 接收完成通知 |
| `nsq.WithDeferredPublish(d)` | 延迟发布 |

### Subscribe 选项

| 选项 | 说明 |
|------|------|
| `nsq.WithConcurrentHandlers(n)` | 并发处理数（默认 1） |
| `nsq.WithMaxInFlight(n)` | 最大在途消息数 |
